...
class Team(mixins.BabelNamed):
    ...
    active_from = models.DateField(verbose_name=_("Active from"),
                                   null=True,
                                   blank=True)
    active_until = models.DateField(verbose_name=_("until"),
                                    null=True,
                                    blank=True)
...
class Patrols(dd.Table):
    help_text = _(
        "A patrol is when a given team works in a given area on a given day.")
    model = Patrol
    parameters = dict(
        dates_from=models.DateField(verbose_name=_("Dates from"),
                                    null=True,
                                    blank=True),
        dates_until=models.DateField(verbose_name=_("until"),
                                     null=True,
                                     blank=True),
    )
    ...
    def get_title_tags(self, ar):
        ...
        if ar.param_values.dates_from:
            if ar.param_values.dates_until:
                yield "%s-%s" % (ar.param_values.dates_from,
                                 ar.param_values.dates_until)
            else:
